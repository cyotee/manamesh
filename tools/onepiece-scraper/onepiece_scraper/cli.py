"""CLI interface for the One Piece card scraper."""
...
from onepiece_scraper.config import AppConfig, load_config
from onepiece_scraper.manifest import validate_manifest
from onepiece_scraper.scraper import Scraper
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="onepiece-scraper",
        description="One Piece TCG card scraper and ManaMesh asset pack builder",
    )
    parser.add_argument(
        "-v", "--verbose",
        action="count",
    ...
    scrape_parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated set IDs to scrape (e.g., OP-01,OP-02)",
    )
    scrape_parser.add_argument(
        "--force",
        action="store_true",
    ...
    if root_manifest.exists():
        ...
        for set_ref in data.get("sets", []):
            ...
            if set_path.exists():
                ...
                errors = validate_manifest(set_data)
                if errors:
                    for e in errors:
