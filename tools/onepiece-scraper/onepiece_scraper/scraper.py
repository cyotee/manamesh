...
from onepiece_scraper.adapters import get_adapter_class
from onepiece_scraper.adapters.base import CardSourceAdapter
from onepiece_scraper.config import AppConfig, SourceConfig
from onepiece_scraper.downloader import ImageDownloader
from onepiece_scraper.manifest import generate_root_manifest, generate_set_manifest
...
class Scraper:
    ...
    def __init__(self, config: AppConfig) -> None:
        ...
        self._downloader = ImageDownloader(config.output.base_dir)

    async def setup(self) -> None:
        """Initialize adapters from config."""
        for src_cfg in self._config.enabled_sources:
            try:
                cls = get_adapter_class(src_cfg.name)
    ...
    async def run(self, force: bool = False, set_filter: Optional[List[str]] = None) -> None:
        ...
        all_cards: Dict[str, List[CardData]] = {}
        ...
        with Progress(
            ...
            for set_id, cards in all_cards.items():
                # Filter out already-downloaded cards (unless force)
                to_download = cards
                if not force:
                    to_download = [
                        c for c in cards
    ...
    async def _discover_sets(self) -> List[SetInfo]:
        ...
        # Add synthetic entries for starters/promos if configured
        if self._config.scrape.include_starters:
            # Common starter set IDs
            for i in range(1, 20):
                sid = f"ST-{i:02d}"
                if sid not in all_sets:
                    all_sets[sid] = SetInfo(id=sid, name=f"Starter Deck {i:02d}", category="starter")

        if self._config.scrape.include_promos:
    ...
    async def _fetch_cards(self, set_id: str, force: bool) -> List[CardData]:
        """Fetch cards from adapters with fallback chain.

        Deduplicates by card ID (keeps first occurrence). APIs often
        return multiple entries per card ID for parallel/alternate art.
        """
        for adapter in self._adapters:
            try:
                cards = await adapter.get_cards(set_id)
                if cards:
                    # Deduplicate by card ID (parallel art variants share IDs)
                    seen: set = set()
                    unique: List[CardData] = []
                    for card in cards:
    ...
    def _get_best_image_url(self, card: CardData) -> str:
        ...
        # Fallback: use the URL stored in card data
