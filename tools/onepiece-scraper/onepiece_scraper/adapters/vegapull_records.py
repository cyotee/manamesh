...
class VegapullRecordsAdapter:
    ...
    The vegapull-records project publishes JSON files with card data
    from GitHub releases. Download the release archive and configure
    `local_path` in config.yaml to point at the extracted directory.
    """
    ...
    def get_image_url(self, card: CardData) -> str:
        """Return the image URL stored in card data (may be empty for local fallback)."""
    ...
    def _extract_set_id(self, raw: Dict[str, Any]) -> Optional[str]:
        """Try to extract a set ID from a vegapull card record."""
        # Try explicit set_id field
        set_id = raw.get("set_id", raw.get("setId", raw.get("set", "")))
        if set_id:
            return str(set_id)
        # Try to derive from card code like "OP01-001"
