"""Image downloader with retry, concurrency, and multi-face support."""
...
class ImageDownloader:
    ...
    Supports multi-face cards (front + back images for MTG DFCs).
    """
    ...
    def __init__(
        ...
        user_agent: str = "ManaMesh-CardScraper/0.2",
    ) -> None:
    ...
    async def download_image(
        self, set_id: str, card_id: str, image_url: str, suffix: str = "",
    ) -> Tuple[bool, Optional[str]]:
        """Download a single image with retry.

        Args:
            set_id: Set code for path construction.
            card_id: Card identifier for filename.
            image_url: URL to download.
            suffix: Optional suffix for multi-face (e.g., "-front", "-back").
        ...
        filename = f"{card_id}{suffix}" if suffix else card_id
        dest = self._output_dir / set_id / "cards" / f"{filename}.{ext}"
            ...
            for attempt in range(1, self._max_retries + 1):
                try:
                    ...
                    if attempt < self._max_retries:
                        ...
                        logger.warning("Download failed for %s: %s", filename, error)
                        return False, error

        return False, "Unknown error"

    async def download_card_image(
        self, card: CardDataBase, image_url: str
    ) -> Tuple[bool, Optional[str]]:
        """Download a single card image (backwards-compatible wrapper)."""
        return await self.download_image(card.set_id, card.id, image_url)

    async def download_batch(
        self,
        cards: List[CardDataBase],
        get_url: callable,
